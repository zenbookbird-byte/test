...
(function () {
  ...
  // Live feed loaded from data/news.json if present. Falls back to
  // the hardcoded NEWS_ITEMS above when the fetch hasn't completed
  // yet, or fails (e.g. when opened from file://).
  let LIVE_NEWS = null;
  let LIVE_NEWS_GENERATED_AT = null;
  ...
  function renderNews() {
    ...
    if (!grid) return;
    // Prefer the live feed (data/news.json, refreshed hourly by the
    // update-news workflow) when it has something to show. Fall back
    // to the static NEWS_ITEMS baked into this bundle.
    let items =
      LIVE_NEWS && LIVE_NEWS.length > 0 ? LIVE_NEWS : NEWS_ITEMS;
  ...
  function initNews() {
    ...
    // Re-check the feed every 10 minutes so a visitor with the tab
    // left open picks up fresh headlines without a manual reload.
    setInterval(loadLiveNews, 10 * 60 * 1000);
  }

  function loadLiveNews() {
    // Cache-bust with the current 10-minute bucket so browsers and
    // Fastly don't serve a stale feed.
    const bucket = Math.floor(Date.now() / (10 * 60 * 1000));
    fetch("data/news.json?t=" + bucket, { cache: "no-cache" })
      .then((r) => (r.ok ? r.json() : null))
      .then((payload) => {
        if (!payload || !Array.isArray(payload.items)) return;
        // Recompute hoursAgo on the client so it stays accurate even
        // if the JSON file is older than the cron interval.
        const now = Date.now();
        LIVE_NEWS = payload.items.map((n) => {
          const pub = n.published ? Date.parse(n.published) : NaN;
          const hoursAgo = isFinite(pub)
            ? Math.max(0, Math.floor((now - pub) / (60 * 60 * 1000)))
            : n.hoursAgo || 0;
          return Object.assign({}, n, { hoursAgo });
        });
        LIVE_NEWS_GENERATED_AT = payload.generated_at || null;
        renderNews();
      })
      .catch(() => {
        /* offline / file:// — keep static fallback */
      });
  }
